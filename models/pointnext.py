...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf):
        p, f = pf     # p: position, f: feature:[]
        if self.is_head:
            f = self.convs(f)  # [b, 4, n]-->[b, c, n] c=32
        else:
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            group_args['radius'] = self.radii[i]  # 直接在字典中进行修改
            group_args['nsample'] = self.nsample[i]  # 直接在字典中进行修改
    ...
    def forward_cls_feat(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            p0, f0 = self.encoder[i]([p0, f0])#f0: [b, 4, n]-->[b, 32, n]-->[b, 64, n/4]-->[b, 128, n/16]-->[b, 256, n/64]-->[b, 512, n/256]
        return f0.squeeze(-1)
